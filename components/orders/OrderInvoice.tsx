  ...
  return (
    <div className="bg-white w-full max-w-2xl mx-auto print:max-w-full">
      ...
      {order.status === "CANCELLED" && (
        <div className="mb-6 print:mb-3 rounded-md border border-red-200 bg-red-50 py-2 text-center">
      ...
      <table className="w-full text-sm mb-6 print:mb-3">
        <thead>
          <tr className="border-y-2 border-gray-900">
            <th className="py-2 print:py-1 text-left text-xs font-bold uppercase tracking-wide">{t("label.item")}</th>
            <th className="py-2 print:py-1 text-center text-xs font-bold uppercase tracking-wide w-12">{t("label.qty")}</th>
            <th className="py-2 print:py-1 text-right text-xs font-bold uppercase tracking-wide w-28">{t("label.unitPrice")}</th>
            <th className="py-2 print:py-1 text-right text-xs font-bold uppercase tracking-wide w-28">{t("label.amount")}</th>
          </tr>
        </thead>
        <tbody>
          {order.orderItems.map((item) => (
            <tr key={item.id} className="border-b border-gray-100">
              ...
              <td className="py-3 print:py-1.5 text-center text-gray-700">{item.quantity}</td>
              <td className="py-3 print:py-1.5 text-right tabular-nums text-gray-700">{formatCurrency(item.unitPrice)}</td>
              <td className="py-3 print:py-1.5 text-right tabular-nums font-semibold text-gray-900">{formatCurrency(item.unitPrice * item.quantity)}</td>
            </tr>
          ))}
        </tbody>
      </table>

      <div className="flex justify-end mb-10 print:mb-4">
